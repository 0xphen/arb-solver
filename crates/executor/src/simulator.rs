...
use super::types::{EdgeUpdate, UpdateStreamer};
...
impl UpdateStreamer for SimulatorStreamer {
    ...
    async fn run_stream(self, sender: Sender<Vec<EdgeUpdate>>) -> Result<(), Error> {
        ...
        loop {
            ...
            let updates: Vec<EdgeUpdate> = (0..self.batch_size)
                .map(|_| {
                    ...
                    EdgeUpdate { from, to, new_rate }
                })
                .collect();

            // Send batch, exit if receiver has been dropped
            if sender.send(updates).await.is_err() {
                eprintln!("Simulator shutting down: Writer receiver dropped.");
...
mod tests {
    ...
    async fn test_node_indices_in_bounds() {
        ...
        for u in updates {
            assert!(u.from < 10, "from node out of bounds");
            assert!(u.to < 10, "to node out of bounds");
            assert!(u.new_rate >= 1.0 - RATE_FLUCTUATION && u.new_rate <= 1.0 + RATE_FLUCTUATION,
                    "rate out of bounds");
        }
    }
}
